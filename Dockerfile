...
#
# Slim Vivliostyle CLI image, assembled with mmdebstrap. Replaces the previous
# Debian-based multi-stage Dockerfile and folds in the former slim/build.sh +
# slim/build-rootfs.sh: a builder stage produces a minimal rootfs directory, and
# the final `FROM scratch` stage is that rootfs plus the image metadata.
#
# BUILD REQUIREMENT: the mmdebstrap step needs the `security.insecure`
# entitlement. mmdebstrap runs a real root chroot and bind-mounts /proc, /sys
# and /dev for the package maintainer scripts, which needs CAP_SYS_ADMIN and an
# unconfined security profile (the same privilege slim/build.sh granted via
# `docker run --cap-add=SYS_ADMIN --security-opt apparmor=unconfined`). BuildKit
# exposes that to a RUN only through `--security=insecure`, so the build needs a
# builder that allows the entitlement:
#
#   docker buildx create --driver docker-container \
#     --buildkitd-flags '--allow-insecure-entitlement security.insecure' --use
#   docker buildx build --allow security.insecure \
#     --build-arg VS_CLI_VERSION=... --build-arg BROWSER=chrome@... .
#
# fakeroot/fakechroot are not an option: their LD_PRELOAD breaks ld.so's RUNPATH
# resolution, so systemd binaries can't find libsystemd-shared. mmdebstrap's
# --mode=unshare (real user namespaces) is blocked by the BuildKit RUN sandbox.

# --- pull the official slim dpkg exclude list ---------------------------------
# debian:trixie-slim ships /etc/dpkg/dpkg.cfg.d/docker -- the upstream "what's
# safe to drop in a container" reference. We copy it out of this stage (replaces
# the `docker run --entrypoint cat` slim/build.sh used) and rewrite it in the
# builder to the form mmdebstrap and dpkg honor.
FROM debian:trixie-slim AS dpkg-excludes

# --- builder: produce the minimal Vivliostyle CLI rootfs ----------------------
# Runs as the TARGET platform (no --platform override) so pnpm pulls the correct
# native binaries for packages like @napi-rs/canvas and @u1f992/mupdf, and so
# @puppeteer/browsers downloads the matching Chrome. Cross-arch builds run this
# stage under emulation (binfmt_misc / docker/setup-qemu-action in CI); same-arch
# runs are native. TARGETARCH is BuildKit's built-in (amd64|arm64) and replaces
# the former TARGET_ARCH.
#
# On amd64 we pre-download Chrome via Puppeteer. On arm64 we install Debian's
# `chromium` package instead because Chrome-for-Testing has no Linux/arm64 build
# (puppeteer#12161); the CLI hardcodes /usr/bin/chromium for linux_arm at
# src/browser.ts.
...
# unzip / xz-utils are required by @puppeteer/browsers to extract the downloaded
# browser archives.
...
# Run NodeSource's setup script here so we can copy its sources list and keyring
# into the target rootfs (the mmdebstrap setup-hooks below replicate the three
# files it writes).
RUN curl --fail --silent --show-error --location https://deb.nodesource.com/setup_24.x --output /tmp/nodesource_setup.sh \
...
# The upstream file uses space-separated 'path-exclude PATTERN' syntax; rewrite
# to 'path-exclude=PATTERN' which mmdebstrap and dpkg honor consistently in
# /etc/dpkg/dpkg.cfg.d.
COPY --from=dpkg-excludes /etc/dpkg/dpkg.cfg.d/docker /tmp/trixie-slim-dpkg-excludes.raw
RUN mkdir --parents /slim \
 && sed 's/^\(path-\(include\|exclude\)\) /\1=/' /tmp/trixie-slim-dpkg-excludes.raw \
        > /slim/trixie-slim-dpkg-excludes.conf

# CLI source. .dockerignore prunes node_modules / tests / examples / .git / slim
# / Dockerfile / ... so this reproduces the worktree set the former build-rootfs.sh
# copied by hand. build/fonts.conf comes in here too (an mmdebstrap hook below
# references it as /src/build/fonts.conf).
COPY . /src

# Build dist/ and assemble the runtime payload. Two separate trees: a full build
# tree (for dist/) and a prod-only tree (for node_modules) -- a prod install over
# a completed full install keeps valibot's optional `typescript` peer; over a
# clean tree it does not.
#
# Then drop native bindings for other platforms (pnpm links them as optional
# deps, none load here). Keep this target's; extend ALL_SUFFIXES when a native
# module adds a platform tuple.
#
...
RUN set -eu; \
    ...
    if [ -d /tmp/runtime/types ]; then cp --archive /tmp/runtime/types /opt/vivliostyle-cli/types; fi; \
...
# Browser packages for the mmdebstrap --include below. On amd64, install $BROWSER
# with the same @puppeteer/browsers the CLI uses and trim it; on arm64, fall back
# to the Debian `chromium` package and let apt resolve its deps. The result is
# stashed in /slim/browser-packages so the mmdebstrap RUN can read it.
#
# These components are bundled only in branded Chrome / CfT builds, not Chromium:
# - WidevineCdm (DRM)
...
#
# UI translations except en-US (LANG=C.UTF-8). Judged safe from the package split
# Alpine adopts:
#   https://gitlab.alpinelinux.org/alpine/aports/-/blob/v3.23.4/community/chromium/APKBUILD#L983-984
#
# Chrome for Testing ships a `deb.deps` file next to each binary, in apt-get
# satisfy format. @puppeteer/browsers' --install-deps reads this same file
# (lib/cjs/install.js:installDeps). The awk normalizes each line: drop version
# specifiers like `(>= 1.2)` (apt --include is name-only), and for `a | b`
# alternatives keep the first variant.
RUN set -eu; \
    if [ "${TARGETARCH}" = "amd64" ]; then \
        ...
        BROWSER_PACKAGES=$(find /opt/puppeteer -name deb.deps -exec cat {} + \
            | awk '{ gsub(/\([^)]*\)/, ""); sub(/ *\|.*/, ""); gsub(/^ +| +$/, ""); if ($0) print }' \
            | sort --unique | tr '\n' ' '); \
    else \
        BROWSER_PACKAGES="chromium"; \
    fi; \
    echo "Browser apt packages: ${BROWSER_PACKAGES}"; \
    printf '%s' "${BROWSER_PACKAGES}" > /slim/browser-packages

# Assemble the rootfs with mmdebstrap. This is the privileged step: --security=insecure
# grants the CAP_SYS_ADMIN + unconfined profile mmdebstrap needs to mount /proc,
# /sys and /dev for the maintainer scripts (see the build-requirement note at the
# top).
RUN --security=insecure set -eu; \
    BROWSER_PACKAGES="$(cat /slim/browser-packages)"; \
    ...
    mmdebstrap \
        ...
        `# from debian:trixie-slim by the dpkg-excludes stage (space-separated lines rewritten to` \
        ...
        --customize-hook='cp /src/build/fonts.conf "$1/etc/fonts/local.conf"' \
...
# --- runtime: the rootfs plus image metadata ----------------------------------
# Plain COPY (no --chown): COPY --from preserves the source's numeric uid/gid, so
# the 1000:1000 ownership survives; a --chown would flatten everything to one
# owner. WORKDIR comes after COPY so /data already exists as 1000:1000. The ENV /
# USER / WORKDIR / ENTRYPOINT below are what slim/build.sh stamped via
