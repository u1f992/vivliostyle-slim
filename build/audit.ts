  ...
  for (let i = 1; i < segs.length; i++) {
    ...
    if (seg.includes('*')) {
      ...
      for (const base of cur) {
        ...
        for (const e of entries) if (re.test(e)) next.push(`${base}/${e}`);
      }
    } else {
      for (const base of cur) next.push(`${base}/${seg}`);
    }
    cur = next;
  }
...
function audit(dl: string): number {
  ...
  if (!installBrowser(dl, 'firefox')) {
    process.stderr.write('audit: could not download firefox for closure analysis\n');
    return 1;
  }
  if (arch === 'amd64' && !installBrowser(dl, 'chromium')) {
    process.stderr.write('audit: could not download chromium for closure analysis\n');
    return 1;
  }
  ...
  const add = (...patterns: string[]) => {
    for (const pat of patterns) for (const m of glob(pat)) seeds.push(m);
  };
  add(
    `${ROOTFS}/usr/bin/node`,
    `${ROOTFS}/usr/bin/npm`,
  ...
  for (const d of [...glob(`${dl}/firefox/*/firefox`), ...glob(`${dl}/chromium/*/chrome-linux`)]) {
    if (!isDir(d)) continue;
    add(`${d}/*.so`);
  }
  ...
  for (const line of runQuiet('ldconfig', ['-C', `${ROOTFS}/etc/ld.so.cache`, '-p']).split('\n')) {
    const arrow = line.lastIndexOf(' => ');
    if (arrow === -1) continue;
    const sn = line.replace(/^\t/, '').split(' ')[0];
    if (!sopath.has(sn)) sopath.set(sn, canon(line.slice(arrow + 4)));
  }
  ...
  for (let i = 0; i < depqueue.length; i++) {
    ...
    if (!pkg || visited.has(pkg)) continue;
    visited.add(pkg);
    for (const dep of dq('-W', '--showformat=${Depends}|${Pre-Depends}\n', pkg)
      ...
      if (r && !visited.has(r)) depqueue.push(r);
    }
  }
  ...
  if (unclassified.length) {
    ...
    process.stderr.write('audit: classify each as keep / purge / purge-packages-late, then rebuild.\n');
    return 1;
  }
  process.stdout.write(`audit: ${candidates.length} purgable packages, all classified.\n`);
  return 0;
}
